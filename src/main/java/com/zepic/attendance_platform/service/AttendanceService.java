...
public class AttendanceService {
    ...
    public AttendanceService(
            AttendanceRecordRepository attendanceRecordRepository, CourseRepository courseRepository,
            StudentRepository studentRepository, CourseEnrollmentRepository courseEnrollmentRepository,
            AppUserRepository appUserRepository, AttendanceMapper attendanceMapper) {
    ...
    @Transactional
    public AttendanceResponse markAttendance(AuthenticatedUser authUser, MarkAttendanceRequest request)
            throws InvalidAttendanceStatusException, AuthenticationRequiredException,
            TeacherRoleRequiredException, CourseNotFoundException, CourseAccessDeniedException, StudentNotFoundException,
        ...
        Long collegeId = TenantContext.get();
        Course course = courseRepository
                .findByIdAndCollege_Id(request.getCourseId(), collegeId)
                .orElseThrow(CourseNotFoundException::new);
        courseRepository
                .findByIdAndCollege_IdAndTeacher_Id(course.getId(), collegeId, teacher.getId())
                .orElseThrow(CourseAccessDeniedException::new);
        Student student = studentRepository
                .findByIdAndCollege_Id(request.getStudentId(), collegeId)
                .orElseThrow(StudentNotFoundException::new);
        if (!courseEnrollmentRepository
                .existsByCourse_IdAndStudent_Id(course.getId(), student.getId())) {
            throw new StudentNotFoundException();
        }
        if (attendanceRecordRepository
                .existsByCourse_IdAndStudent_IdAndAttendanceDate(course.getId(), student.getId(), request.getAttendanceDate())) {
    ...
    public PagedResponse<AttendanceResponse> listAttendanceForTeacher(AuthenticatedUser authUser, Long courseId,
            LocalDate date, Long studentId, String status, Pageable pageable)
            throws AuthenticationRequiredException, TeacherRoleRequiredException, CourseNotFoundException,
            CourseAccessDeniedException {
        AppUser teacher = requireTeacher(authUser);
        Long collegeId = TenantContext.get();
        Course course = courseRepository
                .findByIdAndCollege_Id(courseId, collegeId)
                .orElseThrow(CourseNotFoundException::new);
        courseRepository
                .findByIdAndCollege_IdAndTeacher_Id(course.getId(), collegeId, teacher.getId())
                .orElseThrow(CourseAccessDeniedException::new);
        Page<AttendanceRecord> page =
                attendanceRecordRepository.findForTeacherListing(collegeId, course.getId(), date, studentId, status, pageable);
        return toPagedResponse(page.map(attendanceMapper::toResponse));
    }
    @Transactional(readOnly = true)
    public AttendanceSummaryResponse getAttendanceSummary(AuthenticatedUser authUser, Long courseId, LocalDate date)
            throws AuthenticationRequiredException, TeacherRoleRequiredException, CourseNotFoundException,
            CourseAccessDeniedException {
        AppUser teacher = requireTeacher(authUser);
        Long collegeId = TenantContext.get();
        ...
        courseRepository
                ...
                .orElseThrow(CourseAccessDeniedException::new);
        long totalStudents =
                courseEnrollmentRepository.countByCourse_Id(course.getId());
        long marked =
                attendanceRecordRepository
                        .countByCourse_IdAndAttendanceDate(course.getId(), date);
        long present =
                attendanceRecordRepository
                        .countByCourse_IdAndAttendanceDateAndStatus(course.getId(), date, "PRESENT");
        long absent =
                attendanceRecordRepository
                        .countByCourse_IdAndAttendanceDateAndStatus(course.getId(), date, "ABSENT");
        long late =
                attendanceRecordRepository
                        .countByCourse_IdAndAttendanceDateAndStatus(course.getId(), date, "LATE");
    ...
    @Transactional(readOnly = true)
    public PagedResponse<AttendanceResponse> getStudentAttendanceHistory(AuthenticatedUser authUser, Long studentId, Long courseId,
        ...
        Long collegeId = TenantContext.get();
        Student student = studentRepository
                .findByIdAndCollege_Id(studentId, collegeId)
                .orElseThrow(StudentNotFoundException::new);
        Page<AttendanceRecord> page =
                attendanceRecordRepository.findForStudentHistory(collegeId, student.getId(), courseId, fromDate,
                        toDate, pageable);
        return toPagedResponse(page.map(attendanceMapper::toResponse));
    }
    private AppUser requireTeacher(AuthenticatedUser authUser)
